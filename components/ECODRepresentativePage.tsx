...
export default function ECODRepresentativePage({ domainId }: RepresentativePageProps) {
  ...
  const handleStructureLoaded = () => {
    console.log('Representative domain structure loaded successfully');
  ...
  const handleStructureError = (err: string) => {
    console.error('Error loading representative domain structure:', err);
    setStructureError(err);
    setStructureLoaded(false);
  };

  // Convert domain data to ThreeDMol format
  const getDomainFor3DMol = (): ThreeDMolDomain[] => {
    if (!domain) return [];

    // Parse range to get start/end
    const rangeParts = domain.range.split('-');
    const rangeStart = rangeParts.length >= 2 ? parseInt(rangeParts[0].replace(/[A-Z]:/g, ''), 10) : 0;
    const rangeEnd = rangeParts.length >= 2 ? parseInt(rangeParts[1], 10) : 0;
    ...
    const pseudoDomain: ProteinDomain = {
      ...
      color: "#4285F4",
      description: domain.title || 'Representative Domain'
    };
              ...
              <div className="lg:col-span-1">
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                  <div className="p-3 bg-gray-50 border-b flex justify-between items-center">
                    <h3 className="font-medium">Domain Structure: {domain.structureId}_{domain.chainId}</h3>
                    <div className="flex space-x-1">
                      <button
                        className="p-1 rounded hover:bg-gray-200"
                        title="Reset View"
                        onClick={() => structureViewerRef.current?.reset?.()}
                      ...
                      <button
                        ...
                        title="Export Image"
                        onClick={() => {
                          const dataUrl = structureViewerRef.current?.exportImage?.();
                          if (dataUrl) {
                  ...
                  {/* Real 3D Structure Viewer */}
                  <div className="relative">
                    <ThreeDMolViewer
                      ref={structureViewerRef}
                      pdbId={domain.structureId}
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              {loadingStats ? (
                <div className="flex justify-center py-12">
                  <div className="text-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-2"></div>
                    <div className="text-sm text-gray-600">Loading taxonomy data...</div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="text-sm text-gray-600 mb-2">
                      Domain distribution across {stats.taxonomyDistribution.length} major taxonomic groups
                    </div>
                    <div className="text-2xl font-bold text-gray-900">
                      {stats.taxonomyDistribution.reduce((sum, item) => sum + item.count, 0)} total domains
                    </div>
                  </div>

                  {/* Taxonomy bars */}
                  <div className="space-y-4">
                    {stats.taxonomyDistribution
                      .sort((a, b) => b.count - a.count)
                      .map((taxon, index) => {
                        const total = stats.taxonomyDistribution.reduce((sum, item) => sum + item.count, 0);
                        const percentage = ((taxon.count / total) * 100).toFixed(1);
                        const colors = [
                        ...
                        return (
                          <div key={taxon.name} className="border rounded-lg p-4">
                            <div className="flex justify-between items-center mb-2">
                              <div>
                                <div className="font-medium text-gray-900">{taxon.name}</div>
                                <div className="text-sm text-gray-600">{taxon.count} domains ({percentage}%)</div>
                              </div>
                            </div>
                            ...
                            {taxon.children && taxon.children.length > 0 && (
                              <div className="mt-3 pl-4 space-y-2">
                                ...
                                {taxon.children.length > 5 && (
                                  <div className="text-xs text-gray-500 italic">
                                    + {taxon.children.length - 5} more
                                  </div>
                                )}
                              </div>
                            )}
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <div className="font-medium">No taxonomy data available</div>
                  <div className="text-sm mt-1">Taxonomy information is not available for this representative domain.</div>
                </div>
              )}
            </div>
          )}
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              {loadingStats ? (
                <div className="flex justify-center py-12">
                  <div className="text-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-2"></div>
                    <div className="text-sm text-gray-600">Loading length data...</div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-blue-50 rounded-lg p-4">
                      ...
                      <div className="text-2xl font-bold text-blue-900">{stats.lengthDistribution.min}</div>
                      <div className="text-xs text-blue-600">residues</div>
                    </div>
                    <div className="bg-green-50 rounded-lg p-4">
                      <div className="text-xs text-green-600 font-medium mb-1">Mean</div>
                      <div className="text-2xl font-bold text-green-900">{stats.lengthDistribution.mean.toFixed(1)}</div>
                      <div className="text-xs text-green-600">residues</div>
                    </div>
                    <div className="bg-purple-50 rounded-lg p-4">
                      <div className="text-xs text-purple-600 font-medium mb-1">Median</div>
                      <div className="text-2xl font-bold text-purple-900">{stats.lengthDistribution.median}</div>
                      <div className="text-xs text-purple-600">residues</div>
                    </div>
                    <div className="bg-orange-50 rounded-lg p-4">
                      <div className="text-xs text-orange-600 font-medium mb-1">Maximum</div>
                      <div className="text-2xl font-bold text-orange-900">{stats.lengthDistribution.max}</div>
                      <div className="text-xs text-orange-600">residues</div>
                    </div>
                  </div>

                  {/* Histogram */}
                  {stats.lengthDistribution.bins && stats.lengthDistribution.bins.length > 0 && (
                    <div>
                      <h4 className="font-medium text-gray-700 mb-3">Distribution Histogram</h4>
                      <div className="space-y-2">
                        {(() => {
                          const maxCount = Math.max(...stats.lengthDistribution.bins.map(bin => bin.count));
                          return stats.lengthDistribution.bins.map((bin, index) => {
                            const widthPercent = (bin.count / maxCount) * 100;
                            const colors = [
                              'bg-blue-400', 'bg-blue-500', 'bg-blue-600',
                              'bg-indigo-500', 'bg-purple-500', 'bg-pink-500'
                            ];
                            const color = colors[Math.floor((index / stats.lengthDistribution.bins.length) * colors.length)];

                            return (
                              <div key={bin.range} className="flex items-center gap-3">
                                <div className="w-24 text-sm text-gray-700 font-mono text-right">
                                  {bin.range}
                                </div>
                                <div className="flex-1 relative">
                                  ...
                                  {widthPercent <= 15 && (
                                    <span className="absolute left-full ml-2 top-1/2 transform -translate-y-1/2 text-xs text-gray-600">
                                      {bin.count}
                                    </span>
                                  )}
                                </div>
                ...
                <div className="text-center py-12 text-gray-500">
                  <Info className="h-12 w-12 mx-auto mb-3 text-gray-400" />
                  <div className="font-medium">No length data available</div>
                  <div className="text-sm mt-1">Length distribution information is not available for this representative domain.</div>
                </div>
              )}
            </div>
          )}
